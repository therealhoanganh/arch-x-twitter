...
function groupByTweet(items) {
  ...
  for (const item of items) {
    ...
    if (!post) {
      // Grouped by tweet_id, because that is what the media rows of one post
      // share. Identity for de-duplication is `canonicalId` -- the ORIGINAL on a
      // repost -- so the same post reposted by several archived profiles is one
      // note, not one per reposter.
