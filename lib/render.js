...
// On a REPOST these are two different tweets, and getting them the wrong way
// round pairs one account's handle with another's id:
//
//   tweet_id    the repost itself, owned by whoever reposted
//   retweet_id  the ORIGINAL, owned by `author`
//
// Verified by following the redirects: x.com/<original author>/status/<tweet_id>
// bounces to x.com/<reposter>/status/<tweet_id>, while
// x.com/<original author>/status/<retweet_id> resolves directly. Both return
// 200, so the wrong pairing does not fail -- it just quietly sends you to the
// repost instead of the post.
//
// `retweet_id` is 0, as a NUMBER, on everything that is not a repost. It is not
// absent, and it is not the string "0" -- quoteBigIds only rewrites ids of 16
// digits or more, so this one stays numeric.
function canonicalId(meta) {
  const rt = meta.retweet_id;
  if (rt && String(rt) !== '0') return String(rt);
  return String(meta.tweet_id || meta.id || '');
}
...
// A repost's text is "RT @handle: ...", which repeats what x-author and
// shared-by already say, in the note title as well as the body.
function stripRetweetPrefix(text) {
  return String(text || '').replace(/^RT @\w+:\s*/, '');
}
...
function expandUrls(text, meta) {
  ...
  return stripRetweetPrefix(String(text || '').replace(/\s*https:\/\/t\.co\/\w+\s*$/g, '')).trim();
}
